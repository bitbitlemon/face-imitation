...
class LRNet(nn.Module):
    def __init__(self, feature_size=136, lm_dropout_rate=0.1, rnn_unit=32,
                 ...
                 ablation_mode='gru'):  # 设置 ablation_mode='gru'，去除GRU层
        ...
        self.lstm = nn.LSTM(input_size=cnn_channels, hidden_size=rnn_unit,
                            num_layers=num_layers, dropout=rnn_dropout_rate,
                            batch_first=True, bidirectional=True)

        # Multi-Head Attention and Self-Attention
        self.multihead_attn = nn.MultiheadAttention(embed_dim=rnn_unit * 2, num_heads=num_heads,
    ...
    def forward(self, x):
        ...
        # 跳过GRU层
        gru_out = x  # Skip GRU, directly use CNN output

        # Pass through LSTM Layer
        lstm_out, _ = self.lstm(gru_out, h0_lstm)

        # Multi-Head Attention Layer
        lstm_out = lstm_out.permute(1, 0, 2)  # Transform to (seq_len, batch, feature) for MHA
        attn_out, _ = self.multihead_attn(lstm_out, lstm_out, lstm_out)
        attn_out = attn_out.permute(1, 0, 2)  # Transform back to (batch, seq_len, feature)
